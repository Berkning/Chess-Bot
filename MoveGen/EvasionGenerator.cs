...
public class EvasionGenerator
{
    ...
    public int Generate(ref Span<Move> moves, CheckInfo checkInfo)
    {
        ...
        if (checkInfo.IsSlidingCheck()) //If sliding piece check, we don't know if we're in double check yet
        {
            ...
            if (HasSecondAttacker(checkInfo)) return moveCount; //Double check, so only king moves possible //TODO: Try just removing this, as double check might just be too rare to justify spending time on checking
